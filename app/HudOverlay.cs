...
class HudOverlay : IDisposable
{
    ...
    public Bitmap DrawBitmap()
    {
        ...
        using (var g = Graphics.FromImage(bmp))
        {
            ...
            // en mode edition : cadre jaune + poignee, pour montrer qu'on peut deplacer
            if (editMode)
            {
                using (var eb = new Pen(Color.FromArgb(230, 255, 198, 42), 2f))
                    g.DrawPath(eb, RoundRect(new Rectangle(1, 1, Width - 3, Height - 3), 9));
                using (var hb = new SolidBrush(Color.FromArgb(230, 255, 198, 42)))
                using (var hf = new Font("Segoe UI", 7f, FontStyle.Bold))
                    g.DrawString("⠿ déplacer", hf, hb, Width - 66, 4);
            }
    ...
    /// <summary>
    /// Active/desactive le mode edition. En edition, le HUD capte la souris
    /// et se laisse deplacer ; sinon il est click-through (les clics vont au jeu).
    /// </summary>
    public void SetEditMode(bool on)
    {
        if (hwnd == IntPtr.Zero || editMode == on) return;
        editMode = on;

        int ex = GetWindowLong(hwnd, GWL_EXSTYLE);
        if (on) ex &= ~WS_EX_TRANSPARENT;   // capte la souris
        else    ex |=  WS_EX_TRANSPARENT;   // laisse passer les clics
        SetWindowLong(hwnd, GWL_EXSTYLE, ex);

        if (on) StartDragWatch();
        Render();   // redessine : un liseré signale le mode edition
    }
